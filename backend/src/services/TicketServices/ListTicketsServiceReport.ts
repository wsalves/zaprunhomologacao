...
export default async function ListTicketsServiceReport(
  ...
  select 
    ...
    DATE_FORMAT(tt.createdAt, '%d/%m/%Y %H:%i') as createdAt,
    DATE_FORMAT(tt.finishedAt, '%d/%m/%Y %H:%i') as closedAt,
    diffDateTimePorExtenso(tt.createdAt, coalesce(tt.ratingAt, tt.finishedAt), null, null, null) as supportTime,
    coalesce(ur.rate, 0) NPS
  from Tickets t
  LEFT JOIN (
        SELECT DISTINCT TicketTraking.*
        FROM TicketTraking
