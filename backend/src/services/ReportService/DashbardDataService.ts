  ...
  const query = `
    ...
    traking as (
      select
        ...
        (t.status = 'closed' or tt.finishedAt is not null) finished,
        (tt.userId is null and coalesce(tt.closedAt,tt.finishedAt) is null and t.status = 'pending') pending,        
        (t.status = 'group') _groups,
        (t.isActiveDemand = true) active,
        (t.isActiveDemand = false) passive,
        coalesce(TIMESTAMPDIFF(minute, tt.startedAt , coalesce(tt.ratingAt, tt.finishedAt)), 0) as supportTime,
        coalesce(TIMESTAMPDIFF(minute, tt.startedAt, tt.queuedAt), 0) as waitTime,
        t.status,
        tt.*,
        ct.id contactId
    ...
    counters as (
      select
        ...
        (select count(id) from traking where _groups) supportGroups,
        (
        ...
        (select count(*) from traking where status = 'closed') tickets,
        ...
        (select 
          (100 * count(tt.id)) / NULLIF((select count(*) total from traking tt inner join UserRatings ur on ur.ticketId = tt.ticketId where rated= true),1)
          from traking  tt
        ...
        (select 
          ...
          (
            select sum(nps.promoter) - sum(nps.detractor) from (
              (select 
                (100*count(tt.id))/NULLIF((select count(*) total from traking tt inner join UserRatings ur on ur.ticketId = tt.ticketId where rated= true),1) promoter
                      ,0 detractor
                        from traking tt
                        inner join UserRatings ur on ur.ticketId = tt.ticketId
                    where tt.rated =true 
              ...
              union
              select 
                          0,(100*count(tt.id))/NULLIF((select count(*) total from traking tt inner join UserRatings ur on ur.ticketId = tt.ticketId where rated= true),1)
                        from traking  tt
                        inner join UserRatings ur on ur.ticketId = tt.ticketId
                    where tt.rated =true 
                    and ur.rate < 7)) nps
    ...
    attedants as (
      ...
      select
          (select CONVERT(coalesce(JSON_OBJECT('avgSupportTime', avgSupportTime, 'avgWaitTime', avgWaitTime, 'supportHappening', supportHappening, 'supportPending', supportPending, 'supportGroups', supportGroups, 'leads', leads, 'supportFinished', supportFinished, 'activeTickets', activeTickets, 'passiveTickets', passiveTickets, 'waitRating', waitRating, 'withoutRating', withoutRating, 'withRating',withRating, 'percRating', percRating, 'npsPromotersPerc', npsPromotersPerc, 'npsPassivePerc', npsPassivePerc, 'npsDetractorsPerc', npsDetractorsPerc, 'npsScore', npsScore, 'tickets', tickets), '{}'),  JSON) from counters) as counters,
          (select CONVERT(COALESCE(JSON_ARRAYAGG(JSON_OBJECT('id', id, 'name', name, 'avgSupportTime', avgSupportTime, 'tickets', tickets, 'rating', rating, 'online', online)), '[]'), JSON) from attedants) as attendants`;

  let where = 'where tt.companyId = ?';
  const replacements: any[] = [companyId];

  if (_.has(params, "days")) {
    where += ' and tt.queuedAt >= date_add(current_timestamp(), interval ? day) ';
